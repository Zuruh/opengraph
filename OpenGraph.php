...
/*
  ...
  Edited/Updated by Hashtiv LLC on july 4th, 2018.

  Changelog:
  -July 4th, 2018:
  --Tidy up code for better debugging or future updates. (Program used: Sublime 3)
  --Added Hashtiv OpenGraph meta data instructions.
...
class OpenGraph implements Iterator
{
  ...
  static private function _parse($HTML){
    ...
    foreach ($tags AS $tag) {
      ...
      /*OpenGraph Hashtiv standards*/
        if ($tag->hasAttribute('property') && strpos($tag->getAttribute('property'), 'hashtiv:') === 0){
          $key = strtr($tag->getAttribute('property'), '-:', '__');
          $page->_values[$key] = $tag->getAttribute('content');
        }

        if ($tag->hasAttribute('name') && strpos($tag->getAttribute('name'), 'hashtiv:') === 0){
          $key = strtr($tag->getAttribute('name'), '-:', '__');
          if(array_key_exists($key, $page->_values)){
            if (!array_key_exists($key.'_additional', $page->_values)){
              $page->_values[$key.'_additional'] = array();
